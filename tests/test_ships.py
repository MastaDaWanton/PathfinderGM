...
def test_the_water_is_where_the_world_says_and_the_tree_agrees():
    """The ruling, and at schema 1.5 the world states it outright: `by` is road, sea or
    river on every route. This used to assert that EVERY Aurvantis leg was a crossing,
    which was true when its trade was 100% cross-border and is the distortion that
    prompted the supplier to re-describe it.

    The measurement worth keeping is that the stated word and the continent tree agree —
    checked across both worlds, every route, in both directions. The inference was right;
    it is still the fallback rather than the answer, because being right by luck is not
    the same as being told."""
    legs = [(w, s["id"], leg) for w in (AURVANTIS, PANGRELLA)
            for s in w.play["settlements"] for leg in journey.legs_from(w, s["id"])]
    assert legs
    for world, here, leg in legs:
        assert leg.by in ("road", "sea", "river"), leg
        assert leg.by_sea == (leg.by in ("sea", "river")), leg
        # And the tree says the same thing about every one of them.
        crossed = (journey.continent_of(world, here)
                   != journey.continent_of(world, leg.to_id))
        assert crossed == (leg.by == "sea"), (leg.to_name, leg.by, crossed)
...
def test_a_settlement_is_a_port_when_it_has_somewhere_to_tie_up():
    ...
    **Nothing in the export says a settlement is on the coast.** Aurvantis has 11 ports
    ...
    """
    for world, expect in ((AURVANTIS, 64), (PANGRELLA, 12)):
        ports = [s["name"] for s in world.play["settlements"]
                 if journey.is_port(world, s["id"])]
        assert ports, "no settlement anywhere can put a ship to sea"
        assert len(ports) < expect, "every settlement in the world is a port"
...
def test_a_crossing_from_inland_starts_on_a_road():
    ...
    leg = next(l for s in AURVANTIS.play["settlements"]
               for l in journey.legs_from(AURVANTIS, s["id"])
               if l.by == "sea" and not l.from_port)
...
def test_a_passage_is_counted_in_whole_days_of_the_ship_making_way():
    ...
    leg = next(l for s in AURVANTIS.play["settlements"]
               for l in journey.legs_from(AURVANTIS, s["id"]) if l.by == "sea")
...
def test_a_land_journey_is_untouched():
    ...
    # By the stated word rather than by the continent tree: a river route is inside one
    # continent and is still not a road, which is the distinction `by` exists to make.
    leg = next(l for s in PANGRELLA.play["settlements"]
               for l in journey.legs_from(PANGRELLA, s["id"]) if l.by == "road")
    hours, _m, how = journey.hours_for(leg, 30)
